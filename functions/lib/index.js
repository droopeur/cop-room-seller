...
exports.addShopifyOrders = functions.https.onRequest((request, response) => {
    ...
    for (let order of orders) {
        ...
        if (order['client']['address_line2']) {
            invoice['client']['address2'] = order['client']['address_line2'];
        }
        ;
        if (order['client']['zip']) {
            invoice['client']['zip'] = order['client']['zip'];
        }
        ;
...
exports.exportRaffle = functions.https.onRequest((request, response) => {
            ...
            if (result.success) {
                createLabelForExport(params.address).then(ship => {
                    ...
                    if (shipment['selectedRate']['rate'] < 140) {
                        const mailOptions = {
                            from: '"DO NOT REPLY" <shipping@cop-room.com>',
                            to: params.address.email,
                            subject: 'Shipping Label to coproom',
                            text: shipment.label,
...
exports.onItemCreated = functions.firestore.document('fiches/{ficheId}').onCreate((snap, context) => {
    ...
    if (!fiche.shipment) {
        ...
        const mailOptions = {
            from: '"DO NOT REPLY" <shipping@cop-room.com>',
            to: fiche['mail_paypal'],
            subject: 'Shipping instructions to coproom',
            text: qrcode,
...
exports.addShopifyOrder = functions.https.onRequest((request, response) => {
